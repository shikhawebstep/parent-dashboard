...
const cancellationReasonOptions = [
  {
    value: "Family emergency - cannot attend",
    label: "Family emergency - cannot attend",
  },
...
// ─── Exit-survey question renderer ──────────────────────────────────────────
// Handles the question types seen in the /parent/survey/list response:
// multiple_choice (radio group), rating_10 (1-10 scale), text (free text).
function SurveyQuestion({ question, value, onChange, error }) {
  return (
    <div className="space-y-2">
          ...
          {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
            <button
              ...
              className={`h-9 w-9 rounded-lg border text-sm font-semibold transition ${
                value === n
                  ? "border-[#237FEA] bg-[#237FEA] text-white"
      ...
      {error && <p className="text-[#F04438] text-xs mt-1">{error}</p>}
    </div>
  );
}

export default function CancelMembershipModal({ isOpen, onClose, booking }) {
  // step: 'form' -> the existing cancellation request details
  //       'survey' -> the required exit survey (mandatory before submit)
  const [step, setStep] = useState("form");

  const [loading, setLoading] = useState(false);
  const [reason, setReason] = useState("");
  const { fetchSurveys, surveys } = useCommon();
  ...
  // Per spec: same required exit survey regardless of cancellation path
  // (exceptional circumstances vs payout) — just one published "exit" survey.
  const exitSurvey = useMemo(
    () =>
      (surveys || []).find(
        (s) => s.surveyCategory === "exit" && s.status === "published",
      ),
    [surveys],
  );
  ...
  // Step 1: validate the request details, then move to the exit survey
  // instead of submitting immediately.
  const handleContinueToSurvey = () => {
    const errs = {};
    if (!effectiveDate)
      errs.effectiveDate = "Please select a cancellation effective date";
    ...
    if (exitSurvey && exitSurvey.questions?.length > 0) {
      setStep("survey");
    } else {
      // No exit survey configured — don't block the parent, submit directly.
      submitCancellation();
    }
  };
  ...
  const handleSurveyAnswerChange = (questionId, value) => {
    ...
    if (surveyErrors[questionId]) {
      setSurveyErrors((prev) => ({ ...prev, [questionId]: null }));
    }
  };

  const validateSurvey = () => {
    const errs = {};
    (exitSurvey?.questions || []).forEach((q) => {
      const val = surveyAnswers[q.questionId];
      if (val === undefined || val === null || val === "") {
        errs[q.questionId] = "This question is required";
      }
    });
  ...
  const submitSurveyResponse = async () => {
    ...
    const venueId = booking?.venueId ?? booking?.venue?.id ?? null;

    const raw = JSON.stringify({
      venueId,
      answers: Object.entries(surveyAnswers).map(([questionId, answer]) => ({
        questionId,
        answer,
      })),
    });
    ...
    const response = await fetch(
      `${API_URL}api/parent/survey/${exitSurvey.id}/response`,
      {
        method: "POST",
        headers: myHeaders,
        body: raw,
  ...
  // Step 2: submit the exit survey answers, then the cancellation request,
  // then show the confirmation copy from the spec.
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      showSuccess("Success", "Request submitted, our team will be in touch.");

      setTimeout(() => {
        onClose();
      }, 2000);
    } catch (err) {
      showError("Something Wrong", err?.message || "Failed to send request");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="md:px-8  px-4 pb-8 space-y-6 mt-7 overflow-y-auto">
            ...
            <div className="bg-red-50 border border-red-100 rounded-xl p-4 space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">
                    You currently have{" "}
                    {booking?.paymentPlan?.duration
                      ? Math.max(0, booking.paymentPlan.duration - 1)
                      : 0}{" "}
                    more months left on your membership
                  </h3>
                </div>
              </div>
              ...
              <div className="">
                <div className="flex justify-between text-xs text-gray-500 mb-1">
                  <span>
                    1 of {booking?.paymentPlan?.duration || "N/A"} Months
                  </span>
                  <span className="font-semibold">
                    {booking?.paymentPlan?.duration
                      ? Math.round((1 / booking.paymentPlan.duration) * 100)
                      : 0}
                    %
                  </span>
                </div>
                <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${booking?.paymentPlan?.duration ? (1 / booking.paymentPlan.duration) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className=" text-xs text-gray-500">
                Cancellations are not permitted during the term of your
                membership.
                <br />
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                Cancellations with one months notices are permitted under
                exceptional circumstances.
              </p>
              <p className="text-sm text-gray-600 leading-relaxed">
                If your reason is unexceptional, you still may cancel your
                membership however this would involve a payout of the remaining{" "}
                {booking?.paymentPlan?.duration
                  ? Math.max(0, booking.paymentPlan.duration - 1)
                  : 0}{" "}
            ...
            <div className="space-y-2">
              ...
              {fieldErrors.effectiveDate && (
                <p className="text-[#F04438] text-xs mt-1">
                  {fieldErrors.effectiveDate}
                </p>
              )}
            </div>
              ...
              <Select
                ...
                styles={{
                  control: (base) => ({
                    ...
                    borderColor: fieldErrors.cancellationReason
                      ? "#F04438"
                      : base.borderColor,
                ...
                <Select
                  ...
                  styles={{
                    control: (base) => ({
                      ...base,
                      backgroundColor: "#F9FAFB",
                      borderColor: fieldErrors.selectedStudents
                        ? "#F04438"
                        : base.borderColor,
                      borderRadius: "0.75rem",
                      padding: "2px",
            ...
            {/* Form */}
            <div className="space-y-2">
              <label className="block text-sm font-medium text-[#1B1B1E]">
            ...
            <div className="md:flex justify-between items-center pt-2">
              <p className="text-sm text-[#4B4B56] font-semibold">
          ...
          <div className="md:px-8 px-4 pb-8 space-y-6 mt-7 overflow-y-auto">
            <div className="space-y-2">
              <h3 className="font-bold text-gray-900 text-lg">
                {exitSurvey?.template?.title || exitSurvey?.title}
              </h3>
              {exitSurvey?.template?.description && (
                <p className="text-sm text-gray-600 leading-relaxed">
                  {exitSurvey.template.description}
                </p>
              )}
              <p className="text-xs text-gray-500">
                Please answer these quick questions to complete your
                cancellation request.
              </p>
            </div>

            {(exitSurvey?.questions || []).map((q) => (
              <SurveyQuestion
                key={q.questionId}
                question={q}
                value={surveyAnswers[q.questionId]}
                onChange={(val) => handleSurveyAnswerChange(q.questionId, val)}
                error={surveyErrors[q.questionId]}
              />
            ))}

            <div className="justify-between items-center pt-2">
              <p className="text-sm text-[#4B4B56] font-semibold">
                Our team will get in touch shortly.
              </p>
              <button
                ...
                disabled={loading}
                className={`bg-[#237FEA] mt-4 md:mt-4 text-white px-6 py-2.5 rounded-xl font-semibold transition-colors
                                    ${loading ? "opacity-60 cursor-not-allowed" : "hover:bg-blue-700"}
                                `}
              >
                {loading ? "Submitting..." : "Submit & send request"}
              </button>
            </div>
          </div>
        )}
