...
const safeVenueName = (item) =>
  renderValue(item?.venue?.name || item?.holidayVenue?.name);
...
const Feedback = ({ activeServiceType }) => {
          ...
          {currentItems.map((item, idx) => (
            ...
            >
              <div className="space-y-4">
                ...
                <MobileRow label="Venue" value={safeVenueName(item)} />
      ...
      <div className={`min-h-screen flex flex-col p-4 ${openResolve ? "flex" : "hidden"}`}>
        ...
        <div className="bg-white rounded-2xl w-full max-w-4xl shadow-sm p-6 md:p-8">
          ...
          <div className="divide-y divide-gray-200">
            ...
            <ResolveRow
              label="Venue"
              value={renderValue(
                resolveData?.venue?.name || resolveData?.holidayVenue?.name
              )}
            />
        ...
        <div className="bg-white rounded-2xl w-full max-w-4xl shadow-sm mt-6 p-6 flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div className="flex items-center gap-3">
              {resolveData?.assignedAgent?.profile && (
                <img
                  src={resolveData.assignedAgent.profile}
                  alt="Profile"
                  className="w-10 h-10 rounded-full"
                  onError={(e) => { e.target.src = "/members/dummyuser.png"; }}
                />
              )}
              <span className="text-gray-800 font-semibold">
                {renderAgentName(resolveData?.assignedAgent)}
              </span>
            </div>
          </div>
        </div>
