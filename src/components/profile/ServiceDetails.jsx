...
export default function ServiceDetails({ booking, onBack }) {
    ...
    if (!booking) {
        return (
            <div className="animate-fadeIn p-6 text-center bg-white rounded-[30px] shadow-sm">
                <p className="text-[#717073] font-medium mb-4">
    ...
    const classSchedule = student?.classSchedule || booking?.classSchedule;
    const venue = booking?.classSchedule?.venue || booking?.venue;
    const paymentPlan = booking?.paymentPlan;

    // Safely resolve the active parent
    const activeParent = booking?.parents?.[0];
    const parentDisplayName = safeName(
        activeParent?.parentFirstName,
        activeParent?.parentLastName
    );
    const parentRelation =
        activeParent?.relationChild
            ? ` / ${activeParent.relationChild}`
            : "";

    // Progress bar
    const progressTotal = paymentPlan?.duration || 12;
    const progressFilled = booking?.progressBar?.filledBars ?? 1;
    const progressPercent =
        progressTotal > 0
            ? Math.min(100, Math.round((progressFilled / progressTotal) * 100))
            : 0;

    // Booking source
    const bookingSource = (() => {
        if (booking?.source) return booking.source;
        if (typeof booking?.leads?.source === "string") return booking.leads.source;
        ...
        if (typeof activeParent?.howDidHear === "string") return activeParent.howDidHear;
        return "N/A";
    })();
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input
                                    ...
                                    value={safeValue(student?.studentFirstName, "")}
                            ...
                            <div className="space-y-1">
                                ...
                                <input
                                    ...
                                    value={safeValue(student?.studentLastName, "")}
                                    className="w-full lg:p-3 p-2 border border-[#E2E1E5] rounded-[12px] focus:outline-none focus:border-[#042C89] text-[#383A46] font-medium"
                                    readOnly
                                />
                            </div>
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input
                                    type="text"
                                    value={safeDate(booking?.createdAt)}
                            ...
                            <div className="space-y-1">
                                ...
                                <input
                                    type="text"
                                    value={safeValue(classSchedule?.className)}
                                    className="w-full lg:p-3 p-2 border border-[#E2E1E5] rounded-[12px] focus:outline-none text-[#383A46] font-medium bg-white"
                                    readOnly
                                />
                            </div>
                    ...
                    <div className="bg-[#363E49] rounded-[30px] overflow-hidden text-white shadow-lg flex flex-col lg:p-7 p-3 h-fit">
                        ...
                        <div className="py-6 pb-2 space-y-5">
                            ...
                            <div className="space-y-4 lg:text-[16px] text-[14px]">
                                ...
                                {/* Venue — special pill styling */}
                                <div className="border-b border-[#495362] pb-2">
                                    <p className="text-white 2xl:text-[20px] xl:text-[18px] lg:text-[16px] text-[14px] font-medium mb-1">
                                        Venue
                                    </p>
                                    <span className="bg-[#3B82F6] text-white px-3 py-1 rounded text-xs font-semibold inline-block">
                                        {safeValue(venue?.address || venue?.name)}
                                    </span>
                                </div>
                                ...
                                <DetailRow
                                    label="Monthly Price"
                                    value={safePrice(paymentPlan?.price, "N/A")}
                                />
                                <DetailRow
                                    label="GoCardless ID"
                                    value={booking?.id}
                                ...
                                <DetailRow
                                    ...
                                    value={safeDateLocaleString(booking?.createdAt)}
                                />

                                {/* Progress */}
                                <div className="border-b border-[#495362] pb-3">
                                    <div className="flex justify-between items-end mb-1">
                                        <p className="text-white 2xl:text-[20px] xl:text-[18px] lg:text-[16px] text-[14px] font-bold">
                                            Progress
                                        </p>
                                    </div>
                                    <p className="lg:text-[16px] text-[14px] font-semibold text-[#BDC0C3] mb-2">
                                        {progressFilled} of {progressTotal} Months
                                    </p>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-xs font-bold text-gray-300 text-white">
                                            {progressPercent}%
                                        </p>
                                    </div>
                                </div>
