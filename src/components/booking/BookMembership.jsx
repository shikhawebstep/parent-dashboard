...
import { showErrorToast } from "../../../utils/swalHelper";
...
const rsStyles = (hasError) => ({
  control: (base, state) => ({
    ...
    boxShadow: state.isFocused
      ? `0 0 0 2px ${hasError ? "rgba(229,62,62,0.3)" : "rgba(59,125,246,0.3)"}`
      : "none",
...
const isValidCardNumber = (val = "") =>
  (val || "").replace(/\s/g, "").length === 16;
const isValidCVC = (val = "") => (val || "").length >= 3;
const isValidSortCode = (val = "") =>
  (val || "").replace(/\D/g, "").length === 6;
const isValidAccountNumber = (val = "") =>
...
const BookMembership = () => {
  const token =
    typeof window !== "undefined" ? localStorage.getItem("parentToken") : null;
  ...
  const [tokenValidation, setTokenValidation] = useState(null); // null = not checked yet, otherwise { valid, data }
  const [tokenValidationLoading, setTokenValidationLoading] = useState(false);
  const conversionToken = searchParams.get("token");
  ...
  // If there's no urlBookingId at all, there's nothing to validate — treat as fine.
  // If there IS a urlBookingId, it's only "valid" once profile has loaded AND a match was found.
  const bookingIdValid = !urlBookingId || Boolean(matchedBooking);
  const bookingOwnershipChecked = !urlBookingId || profile !== null;
  ...
  const isReservedBookingFlow = Boolean(
    urlVenueId && urlCreatedAt && !bookingSource,
  );

  // { [tmpId]: 'membership' | 'waitlist' } — per-child choice when a class is over capacity
  const [studentAssignment, setStudentAssignment] = useState({});

  const setAssignment = (tmpId, value) => {
    setStudentAssignment((prev) => ({ ...prev, [tmpId]: value }));
  };
  ...
  const starterPackPrice =
    Number(selectedVenue?.all?.starterPacks?.[0]?.price) || 0;
  ...
  const [starterPackStatusLoading, setStarterPackStatusLoading] =
    useState(false);
    ...
    fetch(`${API_BASE_URL}api/parent/starter-pack-status/${parentId}`, {
      ...
      .then((result) => {
        const hasStarterPack =
          result?.hasStarterPack ?? result?.data?.hasStarterPack ?? false;
  ...
  const venueOptions = (venues?.capacityVenues || [])
    ...
    .map((v) => ({
      value: v.venueId,
      label: v.venueName || "Unnamed venue",
      all: v,
    }));
    ...
    Object.keys(classesObj).forEach((day) => {
      (classesObj[day] || []).forEach((c) => {
        ...
        opts.push({
          value: c.classId,
          label:
            `${c.className || "Class"} (${day}) ${c.time || ""} ${c.level ? `- ${c.level}` : ""}`.trim(),
          all: { ...c, day },
        });
      });
    });
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      ...
      try {
        ...
        const raw = JSON.stringify({ conversionToken });
        ...
        const response = await fetch(
          `${API_BASE_URL}api/parent/validate-token`,
          requestOptions,
        );
        const result = await response.json().catch(() => ({}));

        // Trust `valid` explicitly when present. Only fall back to `status`
        // if the API didn't return a `valid` field at all (e.g. older responses).
        const isValid =
          response.ok &&
          Boolean(
            typeof result?.valid === "boolean" ? result.valid : result?.status,
          );
        ...
        setTokenValidation({
          valid: false,
          message: "Something went wrong validating this link.",
          data: null,
        });
      } finally {
  ...
  useEffect(() => {
    ...
    setStudents(
      normalizedStudents.length ? normalizedStudents : [INIT_STUDENT()],
    );
  ...
  // ── Get ALL over-capacity groups (per selected class) ────────────────────
  const getOverCapacityGroups = () => {
    if (activeStudents.length < 2) return [];

    const byClass = {};
    activeStudents.forEach((s) => {
      if (!s.selectedClassId) return;
      const key = String(s.selectedClassId);
    ...
    Object.values(byClass).forEach((group) => {
      const classData = group[0]?.selectedClassData;
      const cap = Number(classData?.capacity);
      if (classData && !Number.isNaN(cap) && group.length > cap) {
        groups.push({
          classId: String(group[0].selectedClassId),
  ...
  const overCapacityGroups = getOverCapacityGroups();
  const hasOverCapacity = overCapacityGroups.length > 0;

  // ── clears stale choices when the over-capacity groups themselves change ──
  const overCapacitySignature = overCapacityGroups
    .map((g) => `${g.classId}:${g.students.length}:${g.capacity}`)
    .join("|");

  useEffect(() => {
    const relevantIds = new Set(
      overCapacityGroups.flatMap((g) => g.students.map((s) => s._tmpId)),
    );
    setStudentAssignment((prev) => {
      const next = {};
      Object.keys(prev).forEach((id) => {
        if (relevantIds.has(Number(id))) next[id] = prev[id];
      });
      return next;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [overCapacitySignature]);

  const isCapacityResolved = overCapacityGroups.every((group) => {
    const allAssigned = group.students.every(
      (s) => studentAssignment[s._tmpId],
    );
    if (!allAssigned) return false;
    const membershipCount = group.students.filter(
      (s) => studentAssignment[s._tmpId] === "membership",
    ).length;
    return membershipCount <= group.capacity;
  });
  ...
  useEffect(() => {
    ...
    if (match) {
      setSelectedVenue({
        value: match.venueId,
        label: match.venueName || "Unnamed venue",
        all: match,
      });
    }
  }, [venues, urlVenueId]);
  ...
  useEffect(() => {
    ...
    if (bookingStudents.length) {
      ...
      setStudents((prev) => {
        const enriched = prev.map((student) => {
          ...
          const sched =
            match?.classSchedule || match?.holidayClassSchedules || null;
          const classId =
            sched?.classScheduleId ?? sched?.id ?? sched?.classId ?? null;
          return {
            ...student,
            id: match?.id ?? student.id,
    ...
    const bookingEmergency = Array.isArray(booking?.emergency)
      ? booking.emergency[0]
      : booking?.emergency;
  ...
  useEffect(() => {
    const eligible =
      isReservedBookingFlow &&
      showStarterPack &&
      !reservationExpired &&
      bookingIdValid &&
      (!conversionToken || tokenValidation?.valid === true);
  ...
  const calculateAmount = (startDate) => {
    ...
    const starterPack = showStarterPack
      ? Number(starterPackPrice) * studentCount
      : 0;
    ...
    const proRataCost = Math.min(
      Number((proRataLessons * ppl).toFixed(2)) || 0,
      monthlyPrice || 0,
    );
    const isFullMonth =
      (isFirstSelected && proRataLessons >= 3) || proRataCost >= monthlyPrice;
  ...
  const handleCardNumberChange = (e) => {
    ...
    if (cardErrors.cardNumber)
      setCardErrors((errs) => ({ ...errs, cardNumber: "" }));
  };
  ...
  const handleExpiryChange = (e) => {
    ...
    if (cardErrors.expiryDate)
      setCardErrors((errs) => ({ ...errs, expiryDate: "" }));
  };
  ...
  const handleSortCodeChange = (e) => {
    ...
    if (ddErrors.branch_code)
      setDdErrors((errs) => ({ ...errs, branch_code: "" }));
  };

  const handleAccountNumberChange = (e) => {
    const formatted = formatAccountNumber(e?.target?.value);
    setPayment((p) => ({ ...p, account_number: formatted }));
    if (ddErrors.account_number)
      setDdErrors((errs) => ({ ...errs, account_number: "" }));
  };
  ...
  const validateDD = () => {
    ...
    if (!(payment.account_holder_name || "").trim())
      errs.account_holder_name = "Account holder name is required";
    if (!(payment.line1 || "").trim()) errs.line1 = "Address is required";
    if (!(payment.city || "").trim()) errs.city = "City is required";
    if (!isValidSortCode(payment.branch_code || ""))
      errs.branch_code = "Enter a valid 6-digit sort code";
    if (!isValidAccountNumber(payment.account_number || ""))
      errs.account_number = "Enter a valid 8-digit account number";
    if (!payment.authorise)
      errs.authorise = "You must agree to the Direct Debit Guarantee";
    setDdErrors(errs);
  ...
  const validateCard = () => {
    ...
    if (!isValidExpiry(payment.expiryDate || ""))
      errs.expiryDate = "Enter a valid expiry date (MM / YY)";
  ...
  // ── Split students: over-capacity ones go by explicit parent choice,
  // everyone else is confirmed automatically ───────────────────────────────
  const splitStudentsByAssignment = (studentsList) => {
    const overCapacityIds = new Set(
      overCapacityGroups.flatMap((g) => g.students.map((s) => s._tmpId)),
    );
    ...
    studentsList.forEach((s) => {
      if (
        overCapacityIds.has(s._tmpId) &&
        studentAssignment[s._tmpId] === "waitlist"
      ) {
        waitlisted.push(s);
      } else {
        confirmed.push(s);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const parentId = parentData?.id;
      const holderParts = (payment.account_holder_name || "")
        .trim()
        .split(" ")
        .filter(Boolean);
      ...
      const deliveryPostcode =
        selectedAddressData?.postcode || postcode || payment.zipCode || "NA";

      // NEW: work out the actual starter pack price to charge, accounting for the
      // applied discount (e.g. the 50%-off reserved-booking offer or a manual code)
      const starterPackBaseTotal = showStarterPack
        ? Number(starterPackPrice)
        : 0;

      let starterPackDiscountAmount = 0;
      if (showStarterPack && isApplied && appliedDiscount?.data) {
        starterPackDiscountAmount =
          appliedDiscount.data.type === "percentage"
            ? (starterPackBaseTotal * Number(appliedDiscount.data.value)) / 100
            : Number(appliedDiscount.data.finalPrice || 0);
      }
      ...
      // ── 1. Create the membership for students who have a confirmed space ──
      if (membershipStudents.length > 0) {
        const payload = {
          venueId: selectedVenue?.value,
          conversionToken: conversionToken || null,
          startDate: selectedDate,
          totalStudents: membershipStudents.length,
          students: membershipStudents.map(buildStudentPayload),
          parents: parentsPayload,
          starterPack: starterPackFinalPrice, // was: showStarterPack ? starterPackPrice : 0
          starterPackOriginalPrice: showStarterPack ? starterPackBaseTotal : 0, // NEW: audit trail
          ...
          payment: {
            ...
            lastName:
              payment.lastName || holderParts.slice(1).join(" ") || "NA",
      ...
      // ── 2. Add the remaining students (no space) to the waiting list ──
      if (waitlistStudents.length > 0) {
        const waitlistPayload = {
          venueId: selectedVenue?.value,
          totalStudents: waitlistStudents.length,
          students: waitlistStudents.map(buildStudentPayload),
          parents: parentsPayload,
          emergency: emergencyPayload,
        };

        const waitlistRes = await fetch(
          `${API_BASE_URL}api/parent/booking/waiting-list/create/${parentId}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      // ── 3. Compose one clear message covering both outcomes ──
      const confirmedNames = membershipStudents
        .map((s) => s.studentFirstName || "Child")
        .join(", ");
      const waitlistedNames = waitlistStudents
        .map((s) => s.studentFirstName || "Child")
        .join(", ");

      let message;
      if (membershipStudents.length && waitlistStudents.length) {
        message = `${confirmedNames} ${membershipStudents.length > 1 ? "have" : "has"} been booked onto the membership. ${waitlistedNames} ${waitlistStudents.length > 1 ? "have" : "has"} been added to the waiting list, as the class is currently at capacity — we'll be in touch as soon as a space opens up.`;
      } else if (waitlistStudents.length) {
        message = `${waitlistedNames} ${waitlistStudents.length > 1 ? "have" : "has"} been added to the waiting list, as the class is currently at capacity — we'll be in touch as soon as a space opens up.`;
      } else {
        message = "Your membership is confirmed!";
      }
  ...
  const activeNames =
    activeStudents.map((s) => s?.studentFirstName || "Child").join(" & ") ||
    "your child";
  ...
  const handleNewChildDOBChange = (e) => {
    ...
    if (newChildErrors.dateOfBirth)
      setNewChildErrors((errs) => ({ ...errs, dateOfBirth: "" }));
  };

  const validateNewChild = () => {
    const errs = {};
    if (!newChildForm.studentFirstName.trim())
      errs.studentFirstName = "First name is required";
    if (!newChildForm.studentLastName.trim())
      errs.studentLastName = "Last name is required";
    if (!isValidDOB(newChildForm.dateOfBirth))
      errs.dateOfBirth = "Enter a valid date (DD/MM/YYYY)";
    ...
    <div className="min-h-screen booking-page bg-[#f4f6f9] text-[#1f2733] poppins pb-28 sm:pb-16 pt-3">
        ...
        <span
          ...
          onClick={() => {
            if (currentStepIndex > 0)
              setFlowStep(flowStates[currentStepIndex - 1]);
          }}
        >
          <ChevronLeft size={20} />
        </span>
      ...
      <div className="max-w-[1100px] mt-5 mx-auto md:px-6 pt-5 px-2">
        ...
        {urlCreatedAt &&
          !bookingSource &&
          !reservationExpired &&
          bookingOwnershipChecked &&
          (!conversionToken || tokenValidation?.valid === true) && // 👈 this excludes the invalid case
          (bookingIdValid ? (
            (() => {
            ...
            <div className=" mx-auto md:px-6 px-2 mt-4">
              <div className="bg-[#fff5f5] border border-[#feb2b2] text-[#c53030] rounded-[14px] px-5 py-3.5 flex items-center gap-2.5 font-semibold text-[14px]">
                ...
                This booking link doesn't match your account — please make sure
                you're signed in as the parent this link was sent to, or contact
        ...
        {isReservedBookingFlow && reservationExpired && bookingIdValid && (
          <div className=" mx-auto md:px-6 px-2 mt-4">
            <div className="bg-[#fff5f5] border border-[#feb2b2] text-[#c53030] rounded-[14px] px-5 py-3.5 flex items-center gap-2.5 font-semibold text-[14px]">
              <AlertTriangle size={18} />
              Your reservation has expired — the 50% discount is no longer
              available, but you can still continue booking.
            </div>
          </div>
        )}
        {/* Screens */}
        <div className="bg-white mt-6 rounded-[18px] shadow-[0_8px_30px_rgba(20,40,80,0.08)] p-4 md:p-10 md:pt-7">
          ...
          {flowStep === "A" && (
            <div>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-2.5 mb-5 w-full max-w-md mx-auto sm:max-w-none">
                <button className="w-full sm:w-auto font-semibold text-[14px] rounded-[30px] px-4 py-2.5 bg-[#eaf1fe] text-[#3b7df6] border border-[#eaf1fe]">
                  Select an existing child
                </button>
                <button
                  onClick={handleAddStudents}
                  className="w-full sm:w-auto font-semibold text-[14px] rounded-[30px] px-4 py-2.5 bg-white text-[#6b7685] border border-[#e7ebf1]"
                >
                  Add a new child
                </button>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2">
                        ...
                        <div>
                          <div className="text-[14px] text-[#6b7685]">Age</div>
                          <div className="text-[14px] font-semibold">
                            {s.age || "-"}
                          </div>
                        </div>
            ...
            <div>
              ...
              <div className="mb-6">
                  ...
                  <div className="border border-t-0 border-[#e7ebf1] px-5 py-3.5 flex gap-6 flex-wrap bg-[#f4f6f9]">
                    <div className="w-full">
                      <Select
                        ...
                        value={
                          venueOptions.find(
                            (o) => o.value === selectedVenue?.value,
                          ) || null
                        }
                        placeholder={
                          venueOptions.length
                            ? "Select a venue to see available details..."
                            : "No venues available"
                        }
                ...
                {selectedVenue &&
                  ...
                  !reservationExpired &&
                  (() => {
                    const firstClass = activeStudents.find(
                      (s) => s.selectedClassData,
                    )?.selectedClassData;
                    const classDay =
                      firstClass?.day || firstClass?.dayOfWeek || "";
                    const classTime =
                      firstClass?.time ||
                ...
                {activeStudents.map((s, idx) => {
                  ...
                  return (
                    <div
                      key={s._tmpId ?? idx}
                      className={`border border-t-0 border-[#e7ebf1] p-3.5 px-5 flex flex-col gap-3.5 ${idx === activeStudents.length - 1 && !showStarterPack ? "rounded-b-[14px]" : ""}`}
                    >
                          ...
                          <div className="flex items-center gap-2">
                            {!isEditingThis && (
                              ...
                              >
                                ...
                                Coach's verdict: {s?.selectedClassData?.level}{" "}
                                level
                              </div>
                            )}
                          </div>
                        ...
                        <div className="flex items-end gap-3">
                          <div className="flex-1">
                            ...
                            <Select
                              ...
                              value={
                                classOptions.find(
                                  (o) => o.value === s.selectedClassId,
                                ) || null
                              }
                              isDisabled={
                                !selectedVenue || classOptions.length === 0
                              }
                              placeholder={
                                !selectedVenue
              ...
              {hasOverCapacity &&
                overCapacityGroups.map((group) => {
                  const membershipCount = group.students.filter(
                    (s) => studentAssignment[s._tmpId] === "membership",
                  ).length;
                  const overLimit = membershipCount > group.capacity;
                  return (
                    <div
                      key={group.classId}
                      className="border border-[#feb2b2] bg-[#fff5f5] rounded-[14px] p-4 mb-4"
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <AlertTriangle
                          size={18}
                          className="text-[#c53030] shrink-0 mt-0.5"
                        />
                        <div>
                          <div className="font-semibold text-[#c53030] text-[14px] mb-1">
                            Not enough space in {group.className}
                          </div>
                          <div className="text-[14px] text-[#7a3030]">
                            Only {group.capacity} spot
                            {group.capacity !== 1 ? "s" : ""} available for{" "}
                            {group.students.length} children in this class.
                            Choose who gets the membership now — anyone else
                      ...
                      <div className="flex flex-col gap-2">
                        {group.students.map((s) => {
                          const choice = studentAssignment[s._tmpId];
                          return (
                            <div
                              key={s._tmpId}
                      ...
                      <div
                        className={`text-[14px] font-semibold mt-3 ${overLimit ? "text-[#c53030]" : "text-[#6b7685]"}`}
                      >
                        {membershipCount} of {group.capacity} membership spot
                        {group.capacity !== 1 ? "s" : ""} selected
                        {overLimit
                          ? " — please move at least one child to the waiting list."
                          : ""}
                      </div>
                    </div>
                  );
              ...
              {showStarterPack && (
                <>
                  ...
                  <div className="grid md:grid-cols-[200px_1fr] gap-5 border border-[#e7ebf1] rounded-[14px] p-[18px] mb-[14px] items-center">
                    ...
                    <div>
                      ...
                      <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2.5 list-none">
                        {[
                          "Samba T-shirt, shorts & socks",
                          "Drawstring bag",
                          "Skills ball",
                          "Road to Rio skills tracker book",
                        ].map((item, i) => (
                          <li
                            key={i}
                  ...
                  {(() => {
                    ...
                    const starterPackDiscountedTotal = Math.max(
                      starterPackOriginalTotal - starterDiscountAmount,
                      0,
                    );
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                              ...
                              onChange={(e) => {
                                ...
                                if (!val.trim() && eligible) {
                                  setAppliedDiscount({
                                    status: true,
                                    message:
                                      "50% off applied for your reserved booking!",
                                    data: { type: "percentage", value: 50 },
                                  });
                                  setIsApplied(true);
                                  setDiscountSource("auto");
                            ...
                            <button
                              ...
                              disabled={
                                isDiscountLoading || !discountCode.trim()
                              }
                          ...
                          {isChecked &&
                            !isDiscountLoading &&
                            (isApplied ? (
                              <p className="text-[#21b573] text-[14px] mt-1.5 flex items-center gap-1">
                                <Check size={12} />{" "}
                                {appliedDiscount?.message ||
                                  "Discount applied!"}
                              </p>
                            ) : (
                              <p className="text-[#e53e3e] text-[14px] mt-1.5">
                  ...
                  {activeStudents.map((s, idx) => {
                    ...
                    return (
                      <div
                        key={s._tmpId ?? idx}
                        className={`border border-[#e7ebf1] rounded-[10px] p-3 mb-3 flex items-center justify-between gap-4 flex-wrap`}
                      >
                        <div>
                        ...
                        <div className="flex-1 min-w-[150px] max-w-[150px]">
                          <label
                            htmlFor={`size-${s._tmpId}`}
                            className="block text-[11px] uppercase font-semibold text-gray-500"
                          >
                            Kit Size
                          </label>
                          <Select
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const result =
                                await getAddressesByPostcode(postcode);
                              if (result?.success) {
                        ...
                        <div className="mt-3">
                          ...
                          {selectedAddress && (
                            <div className="mt-3 text-[12px] md:text-[14px] text-[#0e7a4d] font-semibold bg-[#e7f8f0] rounded-lg px-3.5 py-2.5 flex items-center gap-2">
                              ...
                              <span
                                className="text-[#3b7df6] cursor-pointer underline ml-1.5"
                                onClick={() => {
                                  setSelectedAddress("");
                                  setSelectedAddressData(null);
                                }}
                      ...
                      {showAddrSelect &&
                        ...
                        !addressLoading && (
                          <div className="mt-3 text-[14px] text-[#8a6d00] bg-[#fffcf0] border border-[#ffd21f] rounded-lg px-3.5 py-2.5 flex items-center gap-2">
                            <MapPin size={16} />
                            No addresses found for this postcode. Try a
                            different one.
                          </div>
                        )}
                    </div>
                ...
                <>
                  ...
                  {paymentPlanOptions.length === 0 ? (
                    <div className="border border-[#ffd21f] bg-[#fffcf0] text-[#8a6d00] font-medium p-4 rounded-[12px] text-[14px] mb-2 flex items-start gap-3">
                      ...
                      <div>
                        No membership plans are available for {numberOfStudents}{" "}
                        student{numberOfStudents > 1 ? "s" : ""} at this venue.
                        <br />
                        <button
                ...
                <>
                  ...
                  {availableDatesSet.size === 0 ? (
                    <div className="border border-[#ffd21f] bg-[#fffcf0] text-[#8a6d00] font-medium p-4 rounded-[12px] text-[14px] mb-6 flex items-start gap-3">
                      <AlertTriangle size={18} className="shrink-0 mt-0.5" />
                      <div>
                        <b>No session dates available</b> at{" "}
                    ...
                    (() => {
                      const today = new Date();
                      today.setHours(0, 0, 0, 0);
                      const hasFutureDates = Array.from(availableDatesSet).some(
                        (dateStr) => {
                      ...
                      return hasFutureDates ? (
                          ...
                          <div className="grid grid-cols-7 gap-1 text-center">
                            ...
                            {Array.from({
                              ...
                              if (isAvail) {
                                if (isPast)
                                  styleClass =
                                    "bg-[#ffebee] text-[#d32f2f] font-semibold cursor-not-allowed";
                                else
                                  styleClass =
                                    "bg-[#eef5ff] text-[#1f2733] font-semibold cursor-pointer hover:bg-[#dcebff]";
                              }
                              if (isSel)
                                styleClass =
                                  "!bg-[#3b7df6] !text-white font-semibold cursor-pointer";
                              return (
                                <div
                                  key={d}
                                  onClick={() => {
                        ...
                        <div className="border border-[#ffd21f] bg-[#fffcf0] text-[#8a6d00] font-medium p-4 rounded-[12px] text-[14px] mb-6 flex items-start gap-3">
                          ...
                          />
                          <div>
                            <b>All session dates have passed</b> at{" "}
                            <b>{selectedVenue?.label || "this venue"}</b>.<br />
                            <button
                              onClick={() => setIsChangingVenue(true)}
                              className="text-[#3b7df6] font-semibold mt-1.5 underline hover:text-[#2f6ae0] block"
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#e7ebf1] p-4 z-40 flex gap-3 w-full sm:relative sm:bottom-auto sm:left-auto sm:right-auto sm:bg-transparent sm:border-t-0 sm:p-0 sm:z-auto justify-center sm:mt-[28px] sm:w-auto">
                ...
                <button
                  disabled={
                    ...
                    (showStarterPack &&
                      !isStarterPackOptional &&
                      activeStudents.some((s) => !studentSizes[s._tmpId])) ||
              ...
              <div className="grid md:grid-cols-[1fr_1.3fr] gap-8">
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      className={inputClass(!!ddErrors.city)}
                      value={payment.city}
                      onChange={(e) => {
                        setPayment((p) => ({ ...p, city: e.target.value }));
                        if (ddErrors.city)
                          setDdErrors((errs) => ({ ...errs, city: "" }));
                      }}
                      placeholder="City"
                  ...
                  <div className="grid grid-cols-2 gap-3.5 mb-2">
                    <div>
                      ...
                      <input
                        className={inputClass(!!ddErrors.branch_code)}
                    ...
                    <div>
                      ...
                      <input
                        className={inputClass(!!ddErrors.account_number)}
                  ...
                  <div>
                    <label
                      className={`flex items-start gap-2.5 text-[14px] cursor-pointer ${ddErrors.authorise ? "text-[#e53e3e]" : "text-[#6b7685]"}`}
                    >
                      <input
                        type="checkbox"
                        ...
                        onChange={(e) => {
                          setPayment((p) => ({
                            ...p,
                            authorise: e.target.checked,
                          }));
                          if (ddErrors.authorise)
                    ...
                    {ddErrors.authorise && (
                      <p className="text-[14px] text-[#e53e3e] mt-1 ml-6">
                        {ddErrors.authorise}
                      </p>
                    )}
                  </div>
          ...
          {flowStep === "E" && (
            <div>
              ...
              <div className="text-center text-[#6b7685] text-[14px] mb-6">
                Your starter pack & delivery are taken today by card — a one-off
                payment
              </div>
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    {cardErrors.cardNumber ? (
                      ...
                      isValidCardNumber(payment.cardNumber) ? (
                      <p className="text-[14px] text-[#21b573] mt-1 flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-2 gap-3.5 mb-4">
                    <div>
                      ...
                      {cardErrors.expiryDate ? (
                        ...
                        isValidExpiry(payment.expiryDate) ? (
                        <p className="text-[14px] text-[#21b573] mt-1">
                          <Check size={12} />
                        </p>
                      ) : null}
                    </div>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      className={inputClass(!!cardErrors.nameOnCard)}
                  ...
                  <div className="grid grid-cols-2 gap-3.5">
                    <div>
                      ...
                      <Select
                        ...
                        value={
                          countryOptions.find(
                            (o) => o.value === payment.country,
                          ) || countryOptions[0]
                        }
                        onChange={(opt) =>
                          setPayment((p) => ({
                            ...p,
                            country: opt?.value || "United Kingdom",
                          }))
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setPayment((p) => ({ ...p, zipCode: e.target.value }))
                        }
                        placeholder="e.g. OX25 4JT"
                      />
                    </div>
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#e7ebf1] p-4 z-40 flex gap-3 w-full sm:relative sm:bottom-auto sm:left-auto sm:right-auto sm:bg-transparent sm:border-t-0 sm:p-0 sm:z-auto justify-center sm:mt-[28px] sm:w-auto">
                ...
                >
                  ...
                  {isSubmitting
                    ? "Processing..."
                    : `Pay £${(pricingBreakdown.totalAmountToday || 0).toFixed(2)} & confirm`}
                </button>
              </div>
            </div>
          )}
          ...
          {flowStep === "D" && (
            <div>
              ...
              <div className="text-center text-[#6b7685] text-[14px] mb-6">
                {submitResultMessage ||
                  `${activeNames}'s membership is confirmed`}
              </div>
              ...
              <div className="max-w-[560px] mx-auto border border-[#e7ebf1] rounded-[16px] overflow-hidden">
                ...
                <div className="p-4">
                  {activeStudents.map((s, i) => (
                    <div
                      key={s._tmpId ?? i}
                      className="flex items-center gap-3.5 py-2.5 font-semibold text-[14px]"
                    >
                      <User size={16} className="text-[#3b7df6]" />{" "}
                      {s.studentFirstName} {s.studentLastName} —{" "}
                      {s.selectedClassData?.className || " (Beginner)"}{" "}
                      {showStarterPack
                        ? `· kit ${studentSizes[s._tmpId] || ""}`
                        : ""}
                    </div>
                  ))}
                  <div className="flex items-center gap-3.5 py-2.5 font-semibold text-[14px]">
                    <CalendarIcon size={16} className="text-[#3b7df6]" /> Starts{" "}
                    {selectedDate || "-"} · {membershipPlan?.label || "Plan"}
                  </div>
                </div>
              </div>
              <div className="max-w-[560px] mx-auto text-[14px] text-[#6b7685] text-start mt-3.5 flex items-center justify-center gap-1.5">
                A confirmation email with your booking details, payment summary
              ...
              <div className="p-8 space-y-6 overflow-y-auto">
                <div className="overflow-x-auto rounded-2xl border border-gray-200 shadow-sm">
                  <table className="w-full text-center border-collapse text-sm">
                    <thead>
                      <tr className="bg-gray-900 text-white text-[14px] font-semibold tracking-wider uppercase">
                        <th
                          rowSpan="2"
                          className="py-3 px-4 border-r border-gray-800 align-middle"
                        >
                          Size
                        </th>
                        <th
                        ...
                        <th
                          colSpan="2"
                          className="py-2 px-4 border-r border-gray-800"
                        >
                          Height
                        </th>
                        <th
                      ...
                      <tr className="bg-gray-800 text-gray-200 text-[14px] font-semibold uppercase">
                        <th className="py-2 px-4 border-r border-gray-700">
                          cm
                        </th>
                        <th className="py-2 px-4 border-r border-gray-700">
                          in
                    ...
                    <tbody className="divide-y divide-gray-200 text-gray-700">
                      {[
                        ...
                        <tr
                          key={idx}
                          className="hover:bg-gray-50/70 transition-colors odd:bg-white even:bg-gray-50/30"
                        >
                          <td className="py-3.5 px-4 font-semibold text-gray-900 border-r border-gray-100">
                            {row.size}
                          </td>
                ...
                <div className="border-l-4 border-[#237FEA] pl-4 py-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    To choose the correct size, measure your child's body as
                    follows:
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (newChildErrors.studentFirstName)
                          setNewChildErrors((errs) => ({
                            ...errs,
                            studentFirstName: "",
                          }));
                    ...
                    />
                    {newChildErrors.studentFirstName && (
                      <p className="text-[14px] text-[#e53e3e] mt-1">
                        {newChildErrors.studentFirstName}
                      </p>
                    )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (newChildErrors.studentLastName)
                          setNewChildErrors((errs) => ({
                            ...errs,
                            studentLastName: "",
                          }));
                    ...
                    />
                    {newChildErrors.studentLastName && (
                      <p className="text-[14px] text-[#e53e3e] mt-1">
                        {newChildErrors.studentLastName}
                      </p>
                    )}
